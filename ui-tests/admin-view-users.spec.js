...
test.beforeEach("Login as admin and navigate to the users page", async ({ page }) => {
    await page.goto(".");
...
test("should display all users", async ({ page }) => {
    ...
    await expect(page.getByTestId(`user-display-item-${users.length-1}`)).toContainText(USER.name);
    await expect(page.getByTestId(`user-display-item-${users.length-1}`)).toContainText(USER.email);
    await expect(page.getByTestId(`user-display-item-${users.length-1}`)).toContainText(USER.phone);
    await expect(page.getByTestId(`user-display-item-${users.length-1}`)).toContainText(USER.address);

})

test("should add new user and display new user in users page", async ({ page, browser }) => {
    const newUser = {
        name: "New User",
        email: "new@email.com",
        password: "password",
    ...
    await newUserPage.getByRole("textbox", { name: "Enter Your Name" }).fill(newUser.name);
    await newUserPage.getByRole("textbox", { name: "Enter Your Email" }).click();
    await newUserPage.getByRole("textbox", { name: "Enter Your Email" }).fill(newUser.email);
    await newUserPage.getByRole("textbox", { name: "Enter Your Password" }).click();
    await newUserPage.getByRole("textbox", { name: "Enter Your Password" }).fill(newUser.password);
    await newUserPage.getByRole("textbox", { name: "Enter Your Phone" }).click();
    await newUserPage.getByRole("textbox", { name: "Enter Your Phone" }).fill(newUser.phone);
    await newUserPage.getByRole("textbox", { name: "Enter Your Address" }).click();
    await newUserPage.getByRole("textbox", { name: "Enter Your Address" }).fill(newUser.address);
    await newUserPage.getByPlaceholder('Enter Your DOB').fill(newUser.DOB);
    await newUserPage.getByRole("textbox", { name: "What is Your Favorite Sport" }).click();
    await newUserPage.getByRole("textbox", { name: "What is Your Favorite Sport" }).fill(newUser.answer);
    ...
    await expect(page.getByTestId(`user-display-item-${users.length-1}`)).toContainText(newUser.name);
    await expect(page.getByTestId(`user-display-item-${users.length-1}`)).toContainText(newUser.email);
    await expect(page.getByTestId(`user-display-item-${users.length-1}`)).toContainText(newUser.phone);
    await expect(page.getByTestId(`user-display-item-${users.length-1}`)).toContainText(newUser.address);
})
